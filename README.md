...
| 设计 | 矢量设计稿：登录页 / 移动屏 / UI 稿 / banner / logo，元素级编辑 + 可加载外框库 |
| PPT | 幻灯片逐页创作与编辑，可直接打开 .pptx 改版式，导出 .pptx |
| 拼豆 | 拼豆图纸：网格绘制、图片转图纸、色卡组、用量统计 |
| 股票 | 行情管线 + 策略系统构建，AI 画线标注，智能盯盘 |
| 桌面 | 窗口实时预览 + 授权范围内的本机操作（观察 / 写草稿 / 发送 / 滚动） |

**Flow 分析画布**是 HTML 画布的 `mode="flow"` 模式：结构化思维可视化，节点连线布局，9 种 ECharts 图表。

所有画布**双向编辑**——AI 可以写，你也可以直接改。历史版本追踪精确记录每一次变更，随时回退。AI 的工作过程从黑箱变成可见——你不需要"相信 AI 会做对"，你看着它做。

### 🖥️ 桌面自动化

泰深可以看你的屏幕，并在你授权的范围内操作其他应用（实验性功能）。

三层授权彼此独立：**监控区**（能看哪一块）、**上传授权**（哪些画面可交给模型）、**操作授权**（动作能落在哪里）。没有授权的区域，泰深既看不到也动不了。

可执行的动作：写草稿、发送消息、滚动、切换会话、关闭弹层、请求用户介入。发送属于提交类动作，默认逐次确认；未获批准时不写入任何输入。

入口：泰案 → 桌面画布。首次使用需选窗口、框出监控区、圈出输入框 / 列表 / 内容区并保存权限。

安全护栏：每个动作都要带上它依据的那一帧画面与区域，画面或窗口几何在之后变化过，动作直接被拒；画布提供紧急停止；出站文本先过本地敏感信息扫描（私钥 / 凭据直接拒绝）。
...
**Skill 自迭代** — Skill 不是一次性的。AI 在使用中发现优化点，直接改写 SKILL.md 正文。Skills 越用越精准。
...
**多 AI 供应商** — 支持 DeepSeek、OpenAI、Mimo、Codex 等多家供应商自由切换。推理强度、上下文窗口等参数下沉到 per-model 级别，不同模型各自微调。

**Codex 订阅集成** — 可切换 **Fast 加速**（按 priority 档位请求；设置页按模型配默认值，聊天框开关只覆盖当前会话，仅 Codex 模型显示）；会话卡片坞会显示 **订阅额度卡片**（剩余额度窗口与重置时间，仅 Codex 会话显示，此场景隐藏本地会话预算卡）。
...
- **6 套主题** — 深海琥珀（默认）/ 极简黑白 / 柔光护眼 / 色码橙 / 原生 macOS / 跟随系统
...
- **taishen_1.7.1.zip** — Windows 解压即用免安装版
